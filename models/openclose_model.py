...
class openclose_model(object):
    ...
    def model_fn(self, features, labels, mode, params):
        """
        Function to create the openclose classification models
        ...
        if type(features) == dict:
            if params['v1_type'] == 'ff_binary':
                ...
                logits, model_params = ff_7l(features['image'],
                                             ...
                                             keep_prob=keep_prob,
                                             num_classes=2)
            elif params['v1_type'] == 'ff_7l_same':
                logits, model_params = ff_7l_same(features['image'],
                                                    labels['label'],
                                                    training=training,
                                                    keep_prob=keep_prob,
                                                    num_classes=2)
                ...
                if params['v1_type'] == 'resnet50':
                    depth = 50
                elif params['v1_type'] == 'resnet18':
                    depth = 18
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            #if params['ft_dense']:
            #    #vars_to_train = [var for var in tf.trainable_variables() if 'linear' in var.name]
            vars_to_train = [var for var in tf.trainable_variables()]
            ...
            #dx = tf.gradients(loss, features['image'])
            #labels_float = tf.cast(labels['label'], tf.float32)
            #dl_close = tf.tensordot(labels_float, dl_dx,1)
            #dl_open = tf.tensordot((1-labels_float), dl_dx, 1) #tf.cast(labels[tf.boolean_mask(dl_dx, (1-labels['label']))
            # dl_close = dl_dx * labels_float #tf.boolean_mask(dl_dx, labels['label'])
            #min_open, max_open = tf.reduce_min(dl_open, (0,1)), tf.reduce_max(dl_open, (0,1))
            #min_close, max_close = tf.reduce_min(dl_close, (0,1)), tf.reduce_max(dl_close, (0,1))
            #dl_open = (dl_open - min_open)/(max_open-min_open)
            #dl_close = (dl_close - min_close)/(max_close-min_close)
            ...
            def host_call_fn(gs, loss, loss_ce, loss_wd, lr, img, kp, file_id): #, dx=None): # dx, file_id):
                ...
                """
                gs = gs[0]
                #open_mean = tf.reduce_mean(dl_open, 0)
                #open_mean = tf.expand_dims(open_mean, 0)
                #close_mean = tf.reduce_mean(dl_close, 0)
                #close_mean = tf.expand_dims(close_mean, 0)
            ...
            #loss_wd_t = tf.reshape(loss_wd, [1])
